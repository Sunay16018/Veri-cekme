...
// HTML ARAYÜZÜ (v9 ile aynı, sadece başlık v10)
const html = `
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8"><title>SkyBot v10 - Hızlı Toplama</title>
    ...
    <style>
        ...
        h3 { color: #58a6ff; margin: 0 0 15px 0; }
...
<body>
    ...
    <div class="main"><div id="log"></div><div style="display:flex; gap:10px;"><input id="msg" style="margin:0;"><button style="background:#1f6feb; width:100px; margin:0;" onclick="send()">YAZ</button></div></div>
    <script>
        const socket = io();
...
io.on('connection', (socket) => {
    socket.on('conn', (data) => {
        ...
        bot.on('login', () => socket.emit('log', '<b style="color:lime">[BAĞLANDI]</b>'));
        bot.on('message', (m) => socket.emit('log', `[CHAT] ${m.toHTML()}`));
    });

    socket.on('disc', () => { if(bot) bot.quit(); automationActive = false; socket.emit('log', 'Bot çıktı.'); });
            ...
            try {
                ...
                if (!hasBlocks) {
                    ...
                    if (chestBlock && chestBlock.name !== 'air') {
                        ...
                        socket.emit('log', 'Hızlı toplama yapılıyor...');
                        // --- HIZLI TOPLAMA MANTIĞI (Shift + Click All) ---
                        for (const item of chest.containerItems()) {
                            if (item.name.includes('block')) {
                                // Mineflayer'ın 'withdraw' fonksiyonu sunucuda Shift+Tık gibi çalışır
                                // null ve item.count diyerek tüm stack'i tek seferde ister
                                try { await chest.withdraw(item.type, null, item.count); } catch(e){}
                                await new Promise(r => setTimeout(r, 150)); // Çok küçük bekleme
                            }
                        }
                        await chest.close();
                    }
                ...
                const blocksInInv = bot.inventory.items().filter(i => i.name.includes('block'));
                if (blocksInInv.length > 0) {
                    await bot.equip(blocksInInv[0], 'hand');
                    ...
                    if (targetBlock && targetBlock.name !== 'air') {
                        ...
                        socket.emit('log', '<b style="color:lime">[TAMAM]</b>');
                    }
                }
            } catch (err) {
                socket.emit('log', 'Hata: ' + err.message);
            }
            if(automationActive) setTimeout(runLoop, 1500);
        };
        runLoop();
    });
});
